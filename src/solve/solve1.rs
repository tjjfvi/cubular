...
pub trait Solve1: Cube + Sized {
  fn solve1(&self) {
    let slice = self.slice(Pos(2, 2, 2), Pos(5, 5, 5));
    slice._solve();
  }
}
...
trait _Solve1: Cube + Sized {
  fn _solve(&self) {
    ...
    let mut todo = self
      ...
      .collect::<Vec<_>>();
    todo.sort_by_key(|x| x.1 .0);
    for (pos, (_, swap)) in todo {
      self._solve_piece(pos, swap, &mut solved);
    }
    let val = self.get_solved(Pos(2, 2, 2));
    self._move_inner(
      self
        .iter()
        .filter(|x| in_inner(x.0))
  ...
  fn _solve_piece(&self, pos: Pos, to_swap: Swap, solved: &mut HashSet<Pos>) {
    ...
    let from = (1..=3)
      ...
      .unwrap();
    let from_swap = self._get_swap(from).1;
  ...
  fn _get_swap(&self, pos: Pos) -> (u8, Swap) {
    match pos {
      p if in_inner(p) => (
        0,
        Swap {
      ...
      p if p.0 >= 3 || p.1 >= 3 => self
        ...
        .tap_mut(|x| x.1.rotate(Axis::Z, -1, 5)),
      p if p.2 >= 1 => {
        let axis = if p.1 == 0 { Axis::X } else { Axis::Y };
        self
          ._get_swap(pos.rotate(axis, 1, 5))
          .tap_mut(|x| x.1.rotate(axis, -1, 5))
      }
      p if p.1 > p.0 => self
        ._get_swap(pos.swap_axes(Axis::X, Axis::Y))
        .tap_mut(|x| x.1.swap_axes(Axis::X, Axis::Y)),
      Pos(0, 0, 0) => (
        0,
        Swap {
          source: Pos(2, 2, 2),
...
impl<T: Cube> _Solve1 for T {}
impl<T: Cube> Solve1 for T {}
